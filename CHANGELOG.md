...
- **Chat-route wiring shipped right after** (`d95a80d` was the editor + migration, this paragraph covers the follow-up wire-in): once the per-user memory commit (`0a5e971`) landed, the chat route gained a 5th `Promise.allSettled` slot for `buildGlobalContext(lastUserMessage)`. Result is wrapped in a `# Operator-curated baseline` markdown header and appended to `combinedContext` after `userMemoryBlock`, so the final system block reads platform → market → freshness → minute → userContext → userMemory → operator baseline. Failure is non-fatal (logs + falls through to empty). Removed the "wiring deferred" amber banner from `/admin/otoole` since edits now flow into live chats.
